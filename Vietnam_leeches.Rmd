...
names(leechidna)
leechidna <- leechidna %>% filter(Total_reads >= lowreadthreshold | Family %in% c("Manidae", "Lorisidae") | Genus %in% c("Panthera") & Total_reads >= 20) # filter out low-read OTUs but keep slow loris and pangolin
...
plot(community.jmds, type = "t")

community2.jmds <- metaMDS(get(community), dist="jaccard")
plot(community2.jmds, type = "t")
