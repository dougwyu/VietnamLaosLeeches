...
DON'T RUN
This makes simpler summary tables, summed over samples subdivided by reservecode2
```{r}
reservecode_by_OTU <- Sample_by_OTU_mammals %>% dplyr::select(Country, reservecode2, Protected_Area_Std_Name, Artiodactyla.Bovidae.Capricornis.milneedwardsii:Soricomorpha.Talpidae.Euroscaptor.UNK)
reservecode_by_OTU$reservecode2 <- as.character(reservecode_by_OTU$reservecode2)
indx <- sapply(reservecode_by_OTU, is.factor)
reservecode_by_OTU[indx] <- lapply(reservecode_by_OTU[indx], function(x) as.numeric(as.character(x)))
reservecode_by_OTU$reservecode2 <- as.factor(reservecode_by_OTU$reservecode2)

reservecode_by_OTU_readsum <- reservecode_by_OTU %>% 
	group_by_at(vars(Country, reservecode2)) %>% 
	summarise_at(vars(Artiodactyla.Bovidae.Capricornis.milneedwardsii:Soricomorpha.Talpidae.Euroscaptor.UNK), sum) 
...
reservecode_by_OTU_b[reservecode_by_OTU_b > 0] <- 1
...
write_tsv(reservecode_by_OTU_readsum, "analysis/reservecode_by_OTU_readsum.tsv")
write_tsv(reservecode_by_OTU_samplecount, "analysis/reservecode_by_OTU_samplecount.tsv")
```
